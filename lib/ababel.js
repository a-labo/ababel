...
async function ababel (pattern, options = {}) {
  ...
  const toRelative = (filename) => path.relative(process.cwd(), filename)
  try {
    const filenames = await aglob(pattern, {cwd, ignore})
    reflects = await aglob(reflects)
    for (const filename of filenames) {
      const src = toRelative(path.resolve(cwd, filename))
      const dest = toRelative(path.resolve(out, ext.reduce((filename, ext) => replaceExt(filename, ext, '.js'), filename)))
      if (!isProduction()) {
